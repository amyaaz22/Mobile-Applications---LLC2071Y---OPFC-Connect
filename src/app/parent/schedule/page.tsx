...
export default function ParentSchedulePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!user) return
      const { data: guardian } = await supabase.from('guardians').select('player:players(category)').eq('profile_id', user.id).single()
      const cat = (guardian?.player as any)?.category ?? 'All'
      setCategory(cat)
      const today = new Date().toISOString().split('T')[0]
      const [{ data: up }, { data: pa }] = await Promise.all([
        supabase.from('training_sessions').select('*').gte('date', today).or(`category.eq.${cat},category.eq.All`).order('date').limit(20),
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
      ...
      {!!past.length && (<><h2 className="section-title mb-3">Past Sessions</h2><div className="space-y-3">{past.map(s => <SessionTile key={s.id} s={s}/>)}</div></>)}
    </div>
  )
}

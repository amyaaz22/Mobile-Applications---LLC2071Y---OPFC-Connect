...
export default function ParentAttendancePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!user) return
      const { data: guardian } = await supabase.from('guardians').select('player_id').eq('profile_id', user.id).single()
      if (!guardian?.player_id) { setLoading(false); return }
      const { data: records } = await supabase.from('attendance').select('*, session:training_sessions(title, date, session_type)').eq('player_id', guardian.player_id).order('scanned_at', { ascending: false })
  ...
  if (!data) return <div className="p-8 text-center text-white/30">No player linked.</div>
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
      ...
      {chartData.length > 0 && <div className="card p-5 mb-6"><h2 className="section-title mb-4">Monthly Attendance</h2><AttendanceChart data={chartData}/></div>}
      <div className="card overflow-hidden">
        <div className="p-4 border-b border-white/5"><h2 className="text-white font-bold text-sm">Session History</h2></div>
        {!records?.length ? <div className="p-8 text-center text-white/30 text-sm">No records yet</div> : (
          <div className="divide-y divide-white/5">
            {records.map((r: any) => (
              <div key={r.id} className="flex items-center gap-3 px-4 py-3">
                {r.status === 'present' ? <CheckCircle size={16} className="text-green-400 flex-shrink-0"/> : r.status === 'late' ? <Clock size={16} className="text-amber-400 flex-shrink-0"/> : <XCircle size={16} className="text-red-400/40 flex-shrink-0"/>}

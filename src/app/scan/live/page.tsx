    ...
    <div className="min-h-screen flex flex-col items-center p-4 pt-6" style={{ background: '#0D1B2A', maxWidth: 480, margin: '0 auto' }}>
      ...
      <div className="relative w-full rounded-2xl overflow-hidden mb-4" style={{ background: '#000', minHeight: 300 }}>
        ...
        <div className="absolute inset-0 pointer-events-none">
          {[['top-4 left-4', 'top-0', 'left-0'], ['top-4 right-4', 'top-0', 'right-0'], ['bottom-4 left-4', 'bottom-0', 'left-0'], ['bottom-4 right-4', 'bottom-0', 'right-0']].map(([pos], i) => (
      ...
      {lastCheckin && (
        <div className={`w-full p-4 rounded-2xl border mb-4 transition-all
          ${lastCheckin.success ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/25 text-xs mt-1">
                {lastCheckin.time.toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
              </p>
            </div>
          </div>
        </div>
      ...
      <p className="text-white/20 text-xs text-center">
        Point camera at player's QR card · 3 second cooldown between scans
      </p>
      <p className="text-white/10 text-xs text-center mt-2">OPFC Connect · Omnis Tactus, Officium</p>
    </div>
  )
}

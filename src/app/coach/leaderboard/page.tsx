...
export default function LeaderboardPage() {
  ...
  async function loadCategories() {
    ...
    const cats = (data?.value as any[])?.map((c: any) => c.name) ?? []
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {leaderboard.map((player, i) => (
            <div key={player.id} className={`card p-4 flex items-center gap-4 border ${rankBg(i)}`}>
              ...
              <div className="flex-1">
                <p className="text-white font-bold">{player.full_name}</p>

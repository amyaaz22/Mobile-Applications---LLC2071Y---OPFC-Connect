...
- **RLS boundary design (schema v12):** mapped against every actual read/write call site in the app before writing a single policy (findings below), rather than gating tables symmetrically by area — a uniform "SELECT and write both require the area" rule would have broken real features. The rules that came out of that trace:
  ...
  - **Write (insert/update/delete) is gated by `has_area_permission(area)`** — this is where the real gap was: a restricted account could previously still mutate any table via a raw API call regardless of what the UI hid from them (the exact class of bug schema v11 fixed for `/api/staff/invite` specifically, generalized here to every table).
  - **Attendance writes are deliberately ungated** by area or category: the two real write paths are the QR scanner (`/scan`, `/scan/live` — intentionally ungated by any permission area today) and the Sessions-detail register (gated by `sessions`, not `attendance` — the dedicated Attendance page is read-only analytics). Gating attendance writes by an area no write path actually requires would only break scanning/session management.
  - **A few tables have a narrow OR-fallback** where one page's write genuinely spans two areas: `payments` insert also allows a `players`-permitted account to create only the `type='entry'` row that player registration/import/trial-conversion auto-creates; `player_points` delete also allows a `players`-permitted account to remove a one-off award from the player detail page; `guardians` writes allow either `players` (player edit page) or `staff` (Staff & Parents' contact management); `profiles` update allows a `players`-permitted account to complete the "auto-link parent account" step (see "How Parent Linking Works") — but *only* by setting `role` to `'parent'` on a row whose role is already `'parent'`, so it can never actually change anyone's role (the `protect_profile_role` trigger's own admin-only check still independently guards every real role/permissions change).
  - Validated by impersonation testing directly against production (`set local role authenticated` + `set_config('request.jwt.claim.sub', ...)`, wrapped in `begin`/`rollback`) against a real narrowed coach account, a real unrestricted admin, and a real parent account before calling it done — not just read from the policy source.
...
every actual call site in the app). Validated against production with
impersonation queries for a real narrowed coach, an unrestricted admin,
and a parent account before shipping. Two accepted, documented gaps from
that design: a `players`-permitted coach can still update *other* fields
(not just `role`) on a `parent`-role profile while completing the
auto-link step (see RBAC notes' `profiles` bullet) — bounded to trusted
staff accounts and to profiles that are already `role='parent'`, so not
worth a trigger-level column-diffing guard for what it prevents; and
`fan_card` was left entirely out of this pass (no `PERMISSION_AREA` maps
to it — it's edited mostly from the parent side, not a gated coach page).
